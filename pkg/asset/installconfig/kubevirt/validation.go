...
func ValidatePlatform(kubevirtPlatform *kubevirt.Platform, machineNetworkEntryList []types.MachineNetworkEntry, clientBuilderFunc ClientBuilderFuncType, fldPath *field.Path) field.ErrorList {
	...
	client, resultErrs := validateInfraClusterReachable(ctx, kubevirtPlatform.InfraClusterKubeConfig, clientBuilderFunc, fldPath)
	allErrs = append(allErrs, resultErrs...)
	if client != nil {
		nsErr := validateNamespaceExistsInInfraCluster(ctx, kubevirtPlatform.InfraClusterNamespace, client, fldPath)
		allErrs = append(allErrs, nsErr...)
		allErrs = append(allErrs, validateStorageClassExistsInInfraCluster(ctx, kubevirtPlatform.InfraClusterStorageClass, client, fldPath)...)
		if len(nsErr) == 0 {
			allErrs = append(allErrs, validateNetworkAttachmentDefinitionExistsInInfraCluster(ctx, kubevirtPlatform.NetworkName, kubevirtPlatform.InfraClusterNamespace, client, fldPath)...)
		}
	}
...
func validateInfraClusterReachable(ctx context.Context, kubeConfigPath string, clientBuilderFunc ClientBuilderFuncType, fieldPath *field.Path) (Client, field.ErrorList) {
	allErrs := field.ErrorList{}
	client, err := clientBuilderFunc(kubeConfigPath)
	if err != nil {
		detailedErr := fmt.Errorf("failed to create InfraCluster client with error: %v", err)
		allErrs = append(allErrs, field.Invalid(fieldPath.Child("InfraClusterReachable"), kubeConfigPath, detailedErr.Error()))
