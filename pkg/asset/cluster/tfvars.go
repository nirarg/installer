...
import (
	...
	kubevirtprovider "github.com/kubevirt/cluster-api-provider-kubevirt/pkg/apis/kubevirtprovider/v1alpha1"
	...
	case kubevirt.Name:
		...
		data, err := kubevirttfvars.TFVars(
			kubevirttfvars.TFVarsSources{
				...
				Kubeconfig:  installConfig.Config.Kubevirt.InfraClusterKubeConfig,
				ImageURL:    string(*rhcosImage),
				Namespace:   installConfig.Config.Kubevirt.InfraClusterNamespace,
			},
		)
		if err != nil {
			return errors.Wrapf(err, "failed to get %s Terraform variables", platform)
